...
GOs_locus = pd.read_csv(file4, keep_default_na=False, na_values=[""], sep="\t", low_memory=False)
GOs_locus.columns = ['index','locus_id','GOs','GO_function','GO_type']
bin_locus = pd.read_csv(file3, keep_default_na=False, na_values=[""], sep="\t", low_memory=False)
bin_locus.columns = ['bin','locus_id']
...
NR_tab_bin = pd.merge(bin_locus[['bin','locus_id']],NR_tab[['locus_id','score','expect','coverage_query','coverage_subject','accession_reference','identities','positives','product']], on='locus_id', how='inner').drop_duplicates()
...
Egg_tab_bin = pd.merge(bin_locus[['bin','locus_id']],Egg_tab[['locus_id','seed_ortholog','evalue','score','eggNOG_OGs','max_annot_lvl','COG_category','Description','Preferred_name','GOs','EC','KEGG_ko','KEGG_Pathway','KEGG_Module','KEGG_Reaction','KEGG_rclass','BRITE','KEGG_TC','CAZy','BiGG_Reaction','PFAMs']], on='locus_id', how='inner').drop_duplicates()

merge = pd.merge(NR_tab_bin[["bin","locus_id","accession_reference","product","identities","coverage_query","coverage_subject"]], Egg_tab[["COG_category","Description","Preferred_name","GOs","KEGG_ko","KEGG_Pathway","CAZy","locus_id"]], on='locus_id', how='outer').drop_duplicates()
...
prod_gr = prod_ko.assign(count=1).groupby(prod_ko['product'].str.lower()).agg({'count':'sum', 'Preferred_name': lambda x: ';'.join(map(str, x)),
...
prod_gr.to_excel(filename2)
filename2b = 'mapping_by_product.tsv'
prod_gr.to_csv(filename2b, sep="\t")
...
filename3c = 'COG_by_locus.xlsx'
cog_kegg_prod_locus.to_excel(filename3c)
filename3d = 'COG_by_locus.tsv'
cog_kegg_prod_locus.to_csv(filename3d, sep="\t")
...
ko_locus_bin = pd.merge(bin_locus[['bin','locus_id']],Knumbers[['locus_id','KEGG_ko']], on='locus_id', how='right').drop_duplicates()
ko_locus_bin_lvl = pd.merge(ko_locus_bin[['bin','locus_id','KEGG_ko']],kegg_levels[['KEGG_ko','level1', 'level2', 'level3', 'ko_description']], on='KEGG_ko', how='left').drop_duplicates()

ko_cog_prod_locus = pd.merge(ko_locus_bin_lvl[['bin','locus_id','KEGG_ko','level1', 'level2', 'level3', 'ko_description']],merge[['locus_id','COG_category','product']], on='locus_id', how='left').drop_duplicates()
...
go_cog_kegg_prod_locus = pd.merge(merge[['bin','locus_id','product','COG_category','KEGG_ko']],GOs_locus[['locus_id','GOs','GO_function','GO_type']], on='locus_id', how='right').drop_duplicates()
